...
def cluster_to_fasta (in_file, 
                      out_file, 
                      ref_seq = None):
    # assert that in_file exists, 
    # otherwise this will crash if tn93-cluster did not run.
        ...
        with open (out_file, "w") as fh2:
            for c in cluster_json:
                ...
                while seq_id in check_uniq:
                        seq_id = cc[0] + '_' + ''.join(random.choices ('0123456789abcdef', 
                                                                       k = 10))
                #end while
         ...
         #end with
    #end with
    return (os.path.getmtime(out_file), 
            len(cluster_json))
#end method
